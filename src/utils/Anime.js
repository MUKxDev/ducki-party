...
export class Anime {
  constructor(baseUrl) {
    this.baseUrl = baseUrl | `https://www1.gogoanime.bid`;
    // if (!this.baseUrl.toLowerCase().startsWith('http'.toLowerCase())) this.baseUrl = "https://" + this.baseUrl
    // if (this.baseUrl.match(/(https?:\/\/)?(www1\.)?(gogoanime.)(ai|vc|bid)/gi) === null) throw new Error('Only 2 supported links currently https://www1.gogoanime.bid and https://gogoanime.vc')
  }
  ...
  async animeInfo(name) {
    ...
    if (typeof name !== "string")
      throw new Error(`Name of the anime should be a string`);
    ...
    try {
      ...
      let nmae = $x(
        "div.js-scrollfix-bottom-rel > article > div.list.di-t.w100"
      )
        .eq(0)
        .find("div.information > a:first")
        .text();

      baseUrl = `${this.baseUrl}//search.html?keyword=${nmae.replace(
        /\s/g,
        "_"
      )}`;
      ...
      let link =
        `${this.baseUrl}` +
        $x("div.last_episodes > ul.items > li")
          .eq(0)
          .find("div.img")
          .find("a")
          .attr("href");
      ...
      let anime = {
        ...
        episodeCount: $a("div.anime_video_body > ul#episode_page > li")
          .find("a")
          .attr("ep_end"),
        link: link,
      };
      ...
      throw new Error(
        `Couldn't fetch the anime info due to an error most probably due to the url or name being Invalid`
      );
    }
  }
  ...
  async animeInfoFetch(link, isUrl) {
    if (!link || typeof link !== "string")
      throw Error(
        "Name or link of the anime must be provided in order to search and should be a STRING format"
      );
    if (!isUrl) isUrl = false;
    if (typeof isUrl !== "boolean")
      throw Error(
        "Typeof Second parameter must be either true or false. Set TRUE to get info by link or set FALSE to get info by name"
      );

    if (link && isUrl === false) {
      let name = link;
      try {
        let baseUrl = `${this.baseUrl}//search.html?keyword=${name.replace(
          /\s/g,
          "_"
        )}`;
        let res = await fetch(baseUrl).then((res) => res.text());
  ...
  async searchAnime(name) {
    ...
    try {
      ...
      $("div.main_body > div.last_episodes > ul.items > li").each(function (
        i,
        elem
      ) {
        $ = cheerio.load($(this).html());
        anime.links.push(
          `https://www1.gogoanime.bid${$("div.img > a").attr("href")}`
        );
      ...
      if (anime.links.length === 0)
        throw Error(
          "Anime fetch failed. Most probably due to invalid anime search name"
        );
      return anime;
    } catch (err) {
      throw new Error(
        `Couldn't search for the anime query most probably due to the BaseUrl or NAME being Invalid`
      );
    }
  }
  ...
  async getEpLinks(url) {
    ...
    let body = await fetch(url)
      ...
      .catch((err) => {
        throw Error(
          "There was an error while fetching the URL. Common reasons: Invalid URL provided"
        );
      });
    let $ = cheerio.load(body);
    ...
    try {
      let epLinks = [];
      let epLenght = $("div.anime_video_body > ul#episode_page > li")
        .find("a")
        .attr("ep_end");
      ...
      for (let c = 1; c < epLenght; c++) {
        epLinks.push(
          `https://www1.gogoanime.bid/${$("div.anime_info_body_bg > h1")
            .text()
            .replace(/ /g, "-")
            .replace(/[^A-Za-z0-9-]/g, "")}-episode-${[c]}`
        );
      }
      ...
      throw Error(
        "Couldn't fetch the episode links for given url. Common reasons: Invalid URL provided"
      );
    }
  }
  ...
  async getInfo(url) {
    ...
    try {
      ...
      throw new Error(
        "Couldn't fetch the anime data most probably due to the URL being Invalid"
      );
    }
  }
}
